...
/**
 * Register the two approved Events detail-route families.
 */
function nice_register_events_rewrite_rules() {
	add_rewrite_rule(
		'^events/services/([^/]+)/?$',
		'index.php?nice_service=$matches[1]',
		'top'
	);
	add_rewrite_rule(
		'^events/case-studies/([^/]+)/?$',
		'index.php?nice_case_study=$matches[1]',
		'top'
	);
}
...
function nice_is_events_content( $post ) {
	...
	return $post instanceof WP_Post
		&& in_array( $post->post_type, array( 'nice_service', 'nice_case_study' ), true )
		&& has_term( 'events', 'nice_division', $post );
}

/**
 * Return the controlled Events URL for a supported content record.
 *
 * @param int|WP_Post $post Post ID or object.
 * @return string
 */
function nice_get_events_content_url( $post ) {
	...
	if ( ! $post instanceof WP_Post || ! nice_is_events_content( $post ) ) {
		return '';
	}

	if ( 'nice_service' === $post->post_type ) {
		return home_url( user_trailingslashit( 'events/services/' . $post->post_name ) );
	}

	if ( 'nice_case_study' === $post->post_type ) {
		return home_url( user_trailingslashit( 'events/case-studies/' . $post->post_name ) );
	}
...
function nice_filter_content_permalink( $permalink, $post ) {
	$events_url = nice_get_events_content_url( $post );

	return $events_url ? $events_url : $permalink;
}
...
/**
 * Enforce the canonical Events paths and keep non-Events CPT records private.
 */
function nice_enforce_content_routes() {
	...
	$canonical = nice_get_events_content_url( $post );
